...
export function SubscriptionsView({
  ...
  const cadenceLabel: Record<string, string> = {
    daily: t("subscriptions.cadenceShort.daily"),
    weekly: t("subscriptions.cadenceShort.weekly"),
    monthly: t("subscriptions.cadenceShort.monthly"),
    quarterly: t("subscriptions.cadenceShort.quarterly"),
    yearly: t("subscriptions.cadenceShort.yearly"),
    custom: t("subscriptions.cadenceShort.custom"),
  };
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            ...
            <span className="font-medium text-foreground">
              ≈ {formatMoney(monthlyTotal)}/{t("subscriptions.perMonth")}
            </span>
          </p>
        </div>
        <SubscriptionDialog
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {formatMoney(s.amount)}/{cadenceLabel[s.cadence] ?? s.cadence}
                    {s.next_due_on
                      ? ` · ${
                          days != null && days >= 0
                            ? t("subscriptions.dueIn", { count: days })
                            : t("subscriptions.dueOn", {
                                date: formatDate(s.next_due_on),
                              })
                        }`
                      : ""}
                  </p>
